...
function getInstallationStaticIp() {
    ...
    .catch((err) => {
        if (err.code === 'ENOENT') {
            logs.warn(`INSTALLATION_STATIC_IP file not found at ${filePath}: ${err.stack || err.message}`);
        } else {
            logs.error(`Error reading INSTALLATION_STATIC_IP ${filePath}: ${err.stack || err.message}`);
        }
        return null;
    });
}
