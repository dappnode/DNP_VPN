...
mkdir -p /opt/src
vpn_provisioning_dir="/usr/share/nginx/html/"
vpn_mobileconfig_tpl="/opt/src/VPN.mobileconfig.tpl"
vpn_mobileconfig="/opt/src/VPN.mobileconfig"
vpn_env_tmp="/opt/src/vpn-gen.env.tmp"
...
echo "VPN_IPSEC_PSK=$VPN_IPSEC_PSK" > "$vpn_env_tmp"
echo "VPN_USER=$VPN_USER" >> "$vpn_env_tmp"
echo "VPN_PASSWORD=$VPN_PASSWORD" >> "$vpn_env_tmp"
chmod 600 "$vpn_env_tmp"
...
echo "PUBLIC_IP=$PUBLIC_IP" >> "$vpn_env_tmp"

cp "$vpn_env_tmp" "$vpn_env"

cp "$vpn_mobileconfig_tpl" "$vpn_mobileconfig"

VPN_IPSEC_PSK_BASE64=$(echo -n $VPN_IPSEC_PSK | base64)
sed -i 's/_VPN_IPSEC_PSK_/'"$VPN_IPSEC_PSK_BASE64"'/g' "$vpn_mobileconfig"
sed -i 's/_VPN_USER_/'"$VPN_USER"'/g' "$vpn_mobileconfig"
sed -i 's/_VPN_PASSWORD_/'"$VPN_PASSWORD"'/g' "$vpn_mobileconfig"
sed -i 's/_PUBLIC_IP_/'"$PUBLIC_IP"'/g' "$vpn_mobileconfig"

cp "$vpn_mobileconfig" "$vpn_provisioning_dir"
...
touch /etc/vpn_env
echo "$PUBLIC_IP" > /etc/vpn_env
echo "$VPN_IPSEC_PSK" > /etc/vpn_env
